...
////// Emissive:
                ...
                float3 node_6343 = (_MainTex_var.rgb*lerp((node_6698_if_leA*_ColorGround.rgb)+(node_6698_if_leB*_ColorSurface.rgb),_ColorSurface.rgb,node_6698_if_leA*node_6698_if_leB));
                float node_8340 = 0.0;
                float4 _FresnelTexture_var = tex2D(_FresnelTexture,TRANSFORM_TEX(i.uv0, _FresnelTexture));
                float3 emissive = (node_6343+(_FresnelColor.rgb*lerp(float3(node_8340,node_8340,node_8340),(pow(1.0-max(0,dot(normalDirection, viewDirection)),_FresnelAmount)*_FresnelTexture_var.rgb),_FresnelStrength)));
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float node_8340 = 0.0;
                float4 _FresnelTexture_var = tex2D(_FresnelTexture,TRANSFORM_TEX(i.uv0, _FresnelTexture));
                o.Emission = (node_6343+(_FresnelColor.rgb*lerp(float3(node_8340,node_8340,node_8340),(pow(1.0-max(0,dot(normalDirection, viewDirection)),_FresnelAmount)*_FresnelTexture_var.rgb),_FresnelStrength)));
